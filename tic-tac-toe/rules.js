...
function begin_play()
{
	playeronename = document.getElementById("player1_id");
	playertwoname = document.getElementById("player2_id");

	if (game_started())
		{
			alert("game in already session")
		}
	...
	if (playeronename == "" || playertwoname == "")
	...
	else
		{
			...
			document.getElementById("turn_info").innerHTML = "Turn for " + whosturn;
			started = true;
...
function reset_play()
{
	start = document.getElementById("turn_info");
	whosturn = document.getElementById("move_text_id");
	playeronename = getElementById("player1_id");
	playertwoname = getElementById("player2_id");
	playeronename.value = ""
	playertwoname.value = ""
	playeronename.disabled = false;
	playeronename.disabled = false;
	whosturn.value = ""
	start.innerHTML = "Game has not begun."
	stared = false;

	for (var i = 0; i < 8; i++)
		{
			x = document.getElementById(table_ids[i])
			x.innerHTML = table_ids[i]
		}
		...
		if (validmove)
			{
				...
				visual = document.getElementById(table_ids[i])
				visual.innerHTML = player
			}
...
function check_winner()
{
	...
	if (board_state[4] != -1){
				...
				if (board_state[4] == board_state[0] && board_state[0] == board_state[8] ){
					winner = board_state[i];
					win_con true;
			}

				if (board_state[4] == board_state[2] && board_state[2] == board_state[6] ){
					winner = board_state[i];
					win_con true;
			}
...
function play()
{
	if (game_started() == false)
		...
		if (turn)
			...
			var mt = document.getElementById("move_text_id").value;
			var i = 0;

			if (mt == "A1" || mt == "A2" || mt == "A3" || mt == "B1" || mt == "B2" || mt == "B3" || mt == "C1" || mt == "C2" || mt == "C3" ||)
				{
					if (mt == "A1")
						{
							i = 0;
				...
				if (turn)
					{
						if (board_state[i] == -1){
							board_state[i] = 1;
							document.getElementById(mt).innerHTML = "X";
							toggle_move();
						}
						else{
					...
					if (board_state[i] == -1){
						board_state[i] = 0;
						document.getElementById(mt).innerHTML = "O";
						toggle_move();
					}
					else{
						alert("cell is already filled")
